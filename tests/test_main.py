...
async def test_get_best_match():
    for mbid in mbid_list:
        ...
        for result in results:
            pass
        #     assertIn('title', result)
        #     assertIs(str, type(result['title']))
        #     assertIn('id', result)
        #     assertIs(str, type(result['id']))
        #     assertRegex(result['id'], r'^[A-z0-9_-]{11}$')


@pytest.mark.asyncio
async def test_empty_match():
...
@pytest.mark.asyncio
async def test_get_yt_search_results():
    await youtube_bz.get_yt_search_results('"Bring Me The Horizon" "MANTRA" "Auto-generated"')


@pytest.mark.asyncio
async def test_get_yt_initial_data():
    yt_search_results = await youtube_bz.get_yt_search_results('"Bring Me The Horizon" "MANTRA" "Auto-generated"')
    await youtube_bz.get_yt_intital_data(yt_search_results)


@pytest.mark.asyncio
async def test_get_search_query():
    release = {
        'artist-credit': [{'name': 'Bring Me The Horizon'}]
    }
    track = {
        'title': 'MANTRA'
    }
...
def test_download(mocker):
    mocker.patch('youtube_dl.YoutubeDL', side_effect=mocked_youtube_dl)
    youtube_bz.download('The Walls Are Way Too Thin', '36hmuTxo88U')
    # self.assertTrue(Path('The Walls Are Way Too Thin.mp3').is_file())


@mock.patch('youtube_bz.run')
def test_argv(mocked_youtube_bz):
